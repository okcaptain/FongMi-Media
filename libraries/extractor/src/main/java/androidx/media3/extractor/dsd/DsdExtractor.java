...
public final class DsdExtractor implements Extractor {
  ...
  public @ReadResult int read(@NonNull ExtractorInput input, @NonNull PositionHolder seekPosition) throws IOException {
    ...
    pendingOutputBytes += parseSector();
    if (pendingOutputBytes >= targetSampleSizeBytes) {
      flushPendingSample();
    }
  ...
  private int parseSector() {
    ...
    offset += frameInfoCount * (dstEncoded == 1 ? FRAME_INFO_SIZE_DST : FRAME_INFO_SIZE_DSD);
    int audioBytes = 0;
    for (int i = 0; i < packetInfoCount; i++) {
