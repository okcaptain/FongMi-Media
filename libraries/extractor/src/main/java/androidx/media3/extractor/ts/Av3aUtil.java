...
/**
 * Utilities for parsing AV3A (AVS3-P3) audio frame headers.
 *
 * <p>Header bit layout (ref: {@code Avs3ParseBsFrameHeader}):
 * <pre>
 *  [12] sync_word == 0xFFF
 *  [ 4] audio_codec_id == 2
 *  [ 1] anc_data_index == 0
 *  [ 3] nn_type_config
 *  [ 3] coding_profile 0=channel, 1=mix, 2=HOA
 *  [ 4] sampling_rate_idx
 *  [ 8] CRC1
 *  profile 0: [7] channel_num_idx
 *  profile 1: [2] sound_bed_type
 *    sbt 0: [7] num_objs+1, [4] bitrate_idx_per_obj
 *    sbt 1: [7] channel_num_idx, [4] bitrate_idx_bed_mc,
 *           [7] num_objs+1, [4] bitrate_idx_per_obj
 *  profile 2: [4] hoa_order+1
 *  [ 2] resolution
 *  profile != 1: [4] bitrate_idx
 *  [ 8] CRC2
 * </pre>
 */
final class Av3aUtil {
  ...
  /**
   * Samples per frame (fixed for all profiles).
   */
  ...
  // Channel config indices (ChannelNumConfig in avs3_stat_com.h)
  ...
  // avs3_rom_com.c: avs3SamplingRateTable
  private static final int[] SAMPLING_RATE_TABLE = {
      192000, 96000, 48000, 44100, 32000, 24000, 22050, 16000, 8000
  };

  // Channel counts per config index; -1 for HOA (computed from order).
  ...
  /**
   * Returns a parsed header, or {@code null} if {@code data} is invalid.
   */
  @Nullable
  static FrameHeader parseFrameHeader(ParsableBitArray data) {
    ...
    data.skipBits(3); // nn_type_config
  ...
  private static boolean buildMixHeader(FrameHeader h, int channelNumIdx, int numObjs, int soundBedType, int bitrateIdxPerObj, int bitrateIdxBedMc) {
    ...
    } else { // soundBedType == 1: MC bed + objects
